...
mapEntriesToKeydir filepath ((offset, Entry _ timestamp ksize vsize key _) : entries) =
  let ksize' = fromIntegral ksize
   in let entry = KeydirEntry filepath vsize (ksize' + 4 + 8 + 8 + 8 + offset) timestamp
       in Map.insert key entry $ mapEntriesToKeydir filepath entries
